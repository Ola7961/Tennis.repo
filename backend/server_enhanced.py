...
from draw_simulator import DrawSimulator, create_wimbledon_draw_structure

app = FastAPI(title="Wimbledon Tennis Predictor API", version="2.0.0")
...
class TennisPredictor:
    ...
    def load_and_process_data(self):
        ...
        # Load all ATP match data files
        data_files = glob.glob('/home/ubuntu/Tennis_repo/data/atp_matches_*.csv')
        all_matches = []
        ...
        if not all_matches:
            raise Exception("No match data files found")
        
        # Combine all data
        df = pd.concat(all_matches, ignore_index=True)
        print(f"Loaded {len(df)} matches from all files")
...
async def startup_event():
    global model, players_data, draw_simulator, tournament_simulation
    
    try:
        # Initialize and train model
        model = TennisPredictor()
        features, labels = model.load_and_process_data()
        training_results = model.train_models(features, labels)
        
        print("Model training completed!")
        print(f"Training results: {training_results}")
        
        # Get players data
        players_data = model.get_players_list()
        print(f"Loaded {len(players_data)} players")
        
        # Initialize draw simulator
        draw_simulator = DrawSimulator(model, players_data)
        
        # Create initial tournament simulation
        draw_structure = create_wimbledon_draw_structure()
        tournament_simulation = draw_simulator.simulate_full_tournament(draw_structure)
        
        print("Tournament simulation completed!")
        print(f"Predicted winner: {tournament_simulation.get('final_winner', 'Unknown')}")
...
async def health_check():
    return {
        ...
        "players_count": len(players_data) if players_data else 0,
        "tournament_simulated": tournament_simulation is not None
    }
...
async def predict_match(request: PredictionRequest):
    ...
    try:
        prediction = model.predict_match(request.player1, request.player2, request.surface)
        return prediction
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@app.get("/api/players")
async def get_players(limit: Optional[int] = 50):
    if not players_data:
        raise HTTPException(status_code=500, detail="Players data not loaded")
    
    limited_players = players_data[:limit] if limit else players_data
    return {"players": limited_players}

@app.get("/api/players/search")
async def search_players(q: str, limit: Optional[int] = 10):
    if not model:
        raise HTTPException(status_code=500, detail="Model not loaded")
    
    try:
        results = model.search_players(q, limit)
...
async def get_wimbledon_draw():
    """Get the Wimbledon draw structure"""
    draw_structure = create_wimbledon_draw_structure()
    return draw_structure

@app.get("/api/wimbledon/simulation")
async def get_tournament_simulation():
    """Get the complete tournament simulation with all predicted matches"""
    if not tournament_simulation:
        raise HTTPException(status_code=500, detail="Tournament simulation not available")
    
    return tournament_simulation

@app.get("/api/wimbledon/probabilities")
async def get_tournament_probabilities():
    """Get tournament win probabilities for all players"""
    if not draw_simulator:
        raise HTTPException(status_code=500, detail="Draw simulator not available")
    
    try:
        draw_structure = create_wimbledon_draw_structure()
        probabilities = draw_simulator.calculate_round_probabilities(
            draw_structure['remaining_players']
        )
        
        # Convert to list format sorted by winner probability
        prob_list = []
        for player, probs in probabilities.items():
            if player in model.players_stats:
                player_stats = model.players_stats[player]
                prob_list.append({
                    'player': player,
                    'current_rank': player_stats['rank'],
                    'tournament_win_probability': round(probs['winner'] * 100, 1),
                    'final_probability': round(probs['final'] * 100, 1),
                    'semi_final_probability': round(probs['semi_final'] * 100, 1),
                    'quarter_final_probability': round(probs['quarter_final'] * 100, 1),
                    'section': 'TBD'  # Could be enhanced to determine section
                })
        
        # Sort by tournament win probability
        prob_list.sort(key=lambda x: x['tournament_win_probability'], reverse=True)
